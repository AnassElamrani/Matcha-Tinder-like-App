...
module.exports = class Like {
  ...
  save() {
    return db.execute(
      'INSERT INTO history(`visitor_id`, `visited_id`) VALUES(?, ?)',
      [this.visitor, this.visited]
    )
  }
  static checkIfVisited(data) {
    return db.execute(
      'SELECT * from history WHERE visitor_id = ? AND visited_id = ?',
      [data.visitor, data.visited]
    )
  }
  ...
  static updateHistoryDate(data) {
    return db.execute(
      'UPDATE `history` SET `created_at` = CURRENT_TIMESTAMP WHERE `visitor_id` = ? AND `visited_id` = ?',
      [data.visitor, data.visited]
    )
  }
}